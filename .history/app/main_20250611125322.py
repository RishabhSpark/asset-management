...
def list_laptops():
    ...
    if show_unassigned:
        ...
        assigned_ids = set([a.laptop_item_id for a in session.query(
            LaptopAssignment).filter_by(unassigned_at=None).all()])
        laptops = session.query(LaptopItem).filter(~LaptopItem.id.in_(
            assigned_ids), LaptopItem.is_retired == False, LaptopItem.is_active == True).all()
    else:
        laptops = session.query(LaptopItem).filter(
...
def assign_laptop():
    ...
    if request.method == "POST":
        ...
        active_assignment = session.query(LaptopAssignment).filter_by(
...
def unassign_laptop_route(assignment_id):
    session = SessionLocal()
    assignment = session.query(LaptopAssignment).filter_by(
...
def add_laptop():
    ...
    if request.method == "POST":
        ...
        if serial and quantity == 1:
            exists = session.query(LaptopItem).filter_by(
...
def edit_laptop(laptop_id):
    ...
    if request.method == "POST":
        ...
        exists = session.query(LaptopItem).filter(
...
def laptop_detail(laptop_id):
    session = SessionLocal()
    laptop = session.query(LaptopItem).options(joinedload(
        LaptopItem.maintenance_logs)).filter_by(id=laptop_id).first()
    assignments = []
    if laptop:
        assignments = session.query(LaptopAssignment).options(joinedload(
...
def replace_device(assignment_id):
    ...
    assigned_ids = set([a.laptop_item_id for a in session.query(
        LaptopAssignment).filter_by(unassigned_at=None).all()])
    unassigned_laptops = session.query(LaptopItem).filter(
...
def download_assignment_history():
    ...
    for assignment in assignments:
        laptop = session.query(LaptopItem).filter_by(
